...
function Acl(module, database) {
    ...
    if (!database.connection.models.Acl) {
        ...
        require('../modules/aclRole')(database);
    }
...
function saveRole(name, role, database) {
    ...
    AclRole.count({role: role}).exec(function (err, count) {
        ...
        row.save(function (err) {
            if (err) {
                return console.log(chalk.yellow('Role:', role, 'has already existed'));
            }
        });
    });
}
    ...
    for (var i in routers) {
        ...
        for (var j in roles) {
            ...
            for (var k in router.allows) {
                funcs.push(this.createIfExists(role, router.allows[k].entity, router.allows[k].permissions, router.allows[k].disabled, false));
            }
        }
    }
...
Acl.prototype.allow = function (role, entity, permissions, disabled, canbedeleted) {
    ...
    var row = new Acl({
        ...
        'auditing.canbedeleted': canbedeleted == 'undefined' ? true : canbedeleted
    });
    row.save(function () {
        deferred.resolve();
    });
