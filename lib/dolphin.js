    ...
    for (var moduleName in  modules) {
        if (modules.hasOwnProperty(moduleName)) {
            ...
            if (module && isFunction(module.angularAppConfigFetcher)) {
                var moduleConf = module.angularAppConfigFetcher();
                if (moduleConf) {
                    appConfig[moduleName] = moduleConf;
                }
            }
        }
    }
...
function Module(name) {
    ...
    // bootstrap models
    var modulePathFolder = modulePath(this.name, 'server');
    if (modulePathFolder) {
        util.walk(modulePathFolder, 'model', null, require);
    }
}
...
/**
 * This is method must be return object with settings for angular app that injected as constant APP_CONFIG.
 * Settings obeject will be accessed by 'moduleName' key
...
Module.prototype.errors = {};

Module.prototype.errorHandlers = [];
...
Dolphinio.prototype.errors = require('./errors/errors');
