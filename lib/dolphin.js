...
Dolphinio.prototype.serve = function (options, callback) {
    ...
    var database = mongoose.connect(defaultConfig.db || '', defaultConfig.dbOptions || {}, function (err) {
        ...
        Dolphinio.Singleton.config = new Config(function (err, config) {
            ...
            //include all services before inject models
            bootstrapServices();
...
function bootstrapServices() {
    util.walkCallback(process.cwd() + '/packages/**/server/services/*', function (file) {
        var obj = require(file)();
        if (obj.name && obj.resolve) {
            Dolphinio.Singleton.addService(obj.name, obj.resolve);
        }
    });
    util.walkCallback(process.cwd() + '/packages/custom/**/server/services/*', function (file) {
        var obj = require(file)();
        if (obj.name && obj.resolve) {
            Dolphinio.Singleton.addService(obj.name, obj.resolve);
        }
...
function bootstrapModels() {
    ...
    util.preload(process.cwd() + '/packages/custom/**/server', 'model');
}
