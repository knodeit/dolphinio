...
Dolphinio.prototype.addService = function (name, callback) {
    container.register('service_' + name, callback);
};

Dolphinio.prototype.getService = function (name) {
    return container.get('service_' + name);
};
...
Dolphinio.prototype.serve = function (options, callback) {
    ...
    var database = mongoose.connect(defaultConfig.db || '', defaultConfig.dbOptions || {}, function (err) {
        if (err) {
            ...
            return console.error('**Could not connect to MongoDB. Please ensure mongod is running and restart dolphin app.**');
        }
        ...
        Dolphinio.Singleton.config = new Config(function (err, config) {
            ...
            Dolphinio.Singleton.events.on('modulesEnabled', function () {
...
Dolphinio.prototype.getActivePackages = function () {
    var deferred = Q.defer();
    var database = container.get('database');
    var Package = database.connection.model('Package');
    Package.find({'settings.disabled': true}, {name: 1}).exec(function (err, rows) {
...
function Module(name) {
    ...
    //load own acl
    util.walk(modulePath(this.name, 'server'), 'acl', '', function (file) {
        $this.aclMatrix = require(file)();

        //load acl
        if ($this.aclMatrix.routers) {
            $this.acl.init($this.aclMatrix.routers);
        }
    });

    //hack for whipp
    if (this.name == 'whipp' || this.name == 'whipp-web') {
        return;
    }

    util.walk(modulePath(this.name, 'server'), 'services', '', function (file) {
        var obj = require(file)($this.config);
        if (obj.enabled && obj.name && obj.resolve) {
            Dolphinio.Singleton.addService(obj.name, obj.resolve);
        }
    });
}
...
Module.prototype.delItemMenu = function (state, callback) {
    var database = container.get('database');
    var Menu = database.connection.model('Menu');
    Menu.remove({state: state}).exec(callback);
};
