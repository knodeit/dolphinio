...
//init global plugins
mongoose.plugin(require('../models_ext/auditing'));
...
function applyMongoosePlugins() {
    var database = container.get('database').connection;
    glob.sync(process.cwd() + '/packages/**/server/models_ext/**/*.js').forEach(function (file) {
        var obj = require(file)();
        if (!obj.model) {
            return;
        }

        if (!database.modelSchemas[obj.model]) {
            return console.log(chalk.yellow('Model', obj.model, 'do not exists for the extension'));
        }

        delete mongoose.connection.models[obj.model];
        mongoose.model(obj.model, database.modelSchemas[obj.model].plugin(obj.plugin));
    });
}
...
var exportJobExecutor = require('./migration/exportJobExecutor');

Module.prototype.exportData = function () {
    return exportJobExecutor.runNewExport.apply(this, arguments);
};

var importJobExecutor = require('./migration/importJobExecutor');
Module.prototype.importData = function () {
    return importJobExecutor.runNewImport.apply(this, arguments);
};
...
Dolphinio.prototype.migration = require('./migration');


Dolphinio.prototype.getEmailTemplateEngine = require('./emailTemplateEngine');



(require('./aggregation'))(Dolphinio);
module.exports = new Dolphinio();
